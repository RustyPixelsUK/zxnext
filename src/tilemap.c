...
void tilemap_init(uint16_t mapdata_offset, uint16_t tiledata_offset, bool enable_512_tiles)
{
	ZXN_WRITE_REG(REG_TILEMAP_CONTROL, TILEMAP_ENABLE | TILEMAP_TILEMAP_OVER_ULA | (enable_512_tiles ? TILEMAP_ENABLE_512_TILES : TILEMAP_NO_ATTRIBUTES));
...
void tilemap_clear(uint8_t tile_id, uint16_t offset, uint16_t length, uint16_t mapdata_offset)
{
	...
	memset((uint8_t *)mapdata_offset + offset, tile_id, length);
}
